#pragma once

#include <tiny-cuda-nn/common.h>
#include <tiny-cuda-nn/common_device.h>
#include <tiny-cuda-nn/encoding.h>
#include <tiny-cuda-nn/gpu_memory.h>
#include <tiny-cuda-nn/multi_stream.h>
#include <tiny-cuda-nn/random.h>

#include <stdexcept>
#include <stdint.h>
#include <string>
#include <vector>

TCNN_NAMESPACE_BEGIN

// static constexpr uint32_t MAX_N_LEVELS = 128;
const float ADJUST_SCALE = std::sqrt(1);

// adjust the cell size as simplex cell is smaller than grid cell
inline uint32_t get_adjusted_base_resolution(uint32_t dim, uint32_t base_resolution) {
	return (uint32_t) (ADJUST_SCALE * base_resolution); // TODO
}

inline uint32_t get_unadjusted_base_resolution(uint32_t dim, uint32_t base_resolution) {
	return (uint32_t) (base_resolution * 1.0f / ADJUST_SCALE); // TODO
}

// number of params in each level
struct SimplexOffsetTable {
	uint32_t data[MAX_N_LEVELS+1] = {};
	uint32_t size = 0;
};

// sort position coord and return the descending index
template <uint32_t N_POS_DIMS>
__device__ void simplex_sort(const float pos[N_POS_DIMS], uint32_t* pos_order) {

	TCNN_PRAGMA_UNROLL
    for (uint32_t i = 0; i < N_POS_DIMS; ++i) {
        pos_order[i] = i;
    }

    // bubble sort
    TCNN_PRAGMA_UNROLL
    for (uint32_t i = 0; i < N_POS_DIMS; ++i) {
    	TCNN_PRAGMA_UNROLL
        for (uint32_t j = i + 1; j < N_POS_DIMS; ++j) {
            if (pos[pos_order[j]] > pos[pos_order[i]]) {
                uint32_t temp = pos_order[i];
                pos_order[i] = pos_order[j];
                pos_order[j] = temp;
            }
        }
    }

}

// hash function
// template <uint32_t N_DIMS, uint32_t N_PRIMES>
// __device__ uint32_t lcg_hash(const uint32_t pos_simplex[N_DIMS], const uint32_t primes[N_PRIMES]) {
// 	static_assert(N_DIMS <= N_PRIMES, "lcg_hash can only hash up to N_PRIMES dimensions.");

// 	uint32_t result = 0;

// 	TCNN_PRAGMA_UNROLL
// 	for (uint32_t i = 0; i < N_DIMS; ++i) {
// 		result ^= pos_simplex[i] * primes[i];
// 	}

// 	return result;
// }

// get hash from simplex position
template <uint32_t N_DIMS>
__device__ uint32_t simplex_hash(const uint32_t pos_simplex[N_DIMS]) {
	constexpr uint32_t factors[7] = { 1u, 2654435761u, 805459861u, 3674653429u, 2097192037u, 1434869437u, 2165219737u };
	return lcg_hash<N_DIMS, 7>(pos_simplex, factors);
}

// calculate index given size, resolution and position
template <uint32_t N_DIMS>
__device__ uint32_t simplex_index(const uint32_t hashmap_size, const uint32_t simplex_resolution, const uint32_t pos_simplex[N_DIMS]) {
	uint32_t stride = 1;
	uint32_t index = 0;

	// The second part of the loop condition is needed to avoid integer overflows in finer levels.
	TCNN_PRAGMA_UNROLL
	for (uint32_t dim = 0; dim < N_DIMS && stride <= hashmap_size; ++dim) {
		index += pos_simplex[dim] * stride;
		stride *= simplex_resolution;
	}

	if (hashmap_size < stride) {
		index = simplex_hash<N_DIMS>(pos_simplex);
	}

	return index % hashmap_size;
}

// __device__ inline float random_val(uint32_t seed, uint32_t idx) {
// 	pcg32 rng{seed};
// 	rng.advance(idx);
// 	return rng.next_float();
// }

// get scale of current level
__host__ __device__ inline float simplex_scale(uint32_t level, float log2_per_level_scale, uint32_t base_resolution) {
	// The -1 means that `base_resolution` refers to the number of simplex _vertices_ rather
	// than the number of cells. This is slightly different from the notation in the paper,
	// but results in nice, power-of-2-scaled parameter simplexes that fit better into cache lines.
	return exp2f(level * log2_per_level_scale) * base_resolution - 1.0f;
}

__host__ __device__ inline uint32_t simplex_resolution(float scale) {
	return (uint32_t)ceilf(scale) + 1;
}

template <typename T, uint32_t N_POS_DIMS, uint32_t N_FEATURES_PER_LEVEL>
__global__ void kernel_simplex(
	const uint32_t num_elements,
	const uint32_t num_simplex_features,
	const SimplexOffsetTable offset_table,
	const uint32_t base_resolution,
	const float log2_per_level_scale,
	const float quantize_threshold,
	float max_level,
	const float* __restrict__ max_level_gpu,
	const InterpolationType interpolation_type,
	const T* __restrict__ simplex,
	MatrixView<const float> positions_in,
	T* __restrict__ encoded_positions,
	float* __restrict__ dy_dx,
	const float f_constant
) {
	const uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i >= num_elements) return;

	const uint32_t level = blockIdx.y; // <- the level is the same for all threads

	if (max_level_gpu) {
		max_level = (max_level_gpu[i] * num_simplex_features) / N_FEATURES_PER_LEVEL;
	} else {
		max_level = (max_level * num_simplex_features) / N_FEATURES_PER_LEVEL;
	}

	if (level >= max_level + 1e-3f) {
		if (encoded_positions) {
			TCNN_PRAGMA_UNROLL
			for (uint32_t f = 0; f < N_FEATURES_PER_LEVEL; ++f) {
				encoded_positions[i + (level * N_FEATURES_PER_LEVEL + f) * num_elements] = (T)0.0f;
			}
		}

		// Gradient is zero for zeroed-out dimensions.
		if (dy_dx) {
			TCNN_PRAGMA_UNROLL
			for (uint32_t f = 0; f < N_FEATURES_PER_LEVEL; ++f) {
				((vector_fullp_t<N_POS_DIMS>*)dy_dx)[i + (level * N_FEATURES_PER_LEVEL + f) * num_elements] = {0};
			}
		}

		return;
	}

	simplex += offset_table.data[level] * N_FEATURES_PER_LEVEL;
	const uint32_t hashmap_size = offset_table.data[level + 1] - offset_table.data[level];

	const float scale = simplex_scale(level, log2_per_level_scale, base_resolution);
	const uint32_t resolution = simplex_resolution(scale);

	float pos[N_POS_DIMS];
	float pos_derivative[N_POS_DIMS];
	uint32_t pos_simplex[N_POS_DIMS];

	float skew = 0;
	TCNN_PRAGMA_UNROLL
	for (uint32_t dim = 0; dim < N_POS_DIMS; ++dim) {
		skew += positions_in(dim, i);
	}
	skew *= f_constant;

	if (interpolation_type == InterpolationType::Nearest || interpolation_type == InterpolationType::Linear) {
		TCNN_PRAGMA_UNROLL
		for (uint32_t dim = 0; dim < N_POS_DIMS; ++dim) {
			pos_fract((positions_in(dim, i) + skew)/std::sqrt(3), &pos[dim], &pos_derivative[dim], &pos_simplex[dim], scale, identity_fun, identity_derivative);
		}
	} else {
		TCNN_PRAGMA_UNROLL
		for (uint32_t dim = 0; dim < N_POS_DIMS; ++dim) {
			pos_fract((positions_in(dim, i) + skew)/std::sqrt(3), &pos[dim], &pos_derivative[dim], &pos_simplex[dim], scale, smoothstep, smoothstep_derivative);
		}
	}

	// get features at simplex vertex
	auto simplex_val = [&](const uint32_t local_pos[N_POS_DIMS]) {
		const uint32_t index = simplex_index<N_POS_DIMS>(hashmap_size, resolution, local_pos) * N_FEATURES_PER_LEVEL;
		return *(vector_t<T, N_FEATURES_PER_LEVEL>*)&simplex[index];
	};

	if (interpolation_type == InterpolationType::Nearest) {
		auto result = simplex_val(pos_simplex);

		if (encoded_positions) {
			TCNN_PRAGMA_UNROLL
			for (uint32_t f = 0; f < N_FEATURES_PER_LEVEL; ++f) {
				encoded_positions[i + (level * N_FEATURES_PER_LEVEL + f) * num_elements] = result[f];
			}
		}

		// Gradient is zero when there's no interpolation.
		if (dy_dx) {
			TCNN_PRAGMA_UNROLL
			for (uint32_t f = 0; f < N_FEATURES_PER_LEVEL; ++f) {
				((vector_fullp_t<N_POS_DIMS>*)dy_dx)[i + (level * N_FEATURES_PER_LEVEL + f) * num_elements] = {0};
			}
		}

		return;
	}

	if (encoded_positions) {
		// N-linear interpolation
		vector_t<T, N_FEATURES_PER_LEVEL> result = {};

		uint32_t pos_order[N_POS_DIMS];
		simplex_sort<N_POS_DIMS>(pos, pos_order);

		float start_weight = 1;
		uint32_t pos_simplex_local[N_POS_DIMS];

		TCNN_PRAGMA_UNROLL
		for (uint32_t idx = 0; idx < N_POS_DIMS; ++idx) {
			pos_simplex_local[idx] = pos_simplex[idx];
		}

		TCNN_PRAGMA_UNROLL
		for (uint32_t idx = 0; idx < N_POS_DIMS + 1; ++idx) {
			float weight = 0;
			auto val = simplex_val(pos_simplex_local);

			if (idx == N_POS_DIMS) {
				weight = start_weight;
			} else {
				weight = start_weight - pos[pos_order[idx]];
				pos_simplex_local[pos_order[idx]] += 1; // next simplex vertex

				start_weight = pos[pos_order[idx]];
			}

			TCNN_PRAGMA_UNROLL
			for (uint32_t feature = 0; feature < N_FEATURES_PER_LEVEL; ++feature) {
				float data = (float)((T*)&val)[feature];
				if (fabsf(data) < quantize_threshold) data = 0.f;
				((T*)&result)[feature] += (T)(weight * data);
			}
		}

		TCNN_PRAGMA_UNROLL
		for (uint32_t f = 0; f < N_FEATURES_PER_LEVEL; ++f) {
			encoded_positions[i + (level * N_FEATURES_PER_LEVEL + f) * num_elements] = result[f];
		}
	}

	// Gradient
	if (dy_dx) {
		// throw std::runtime_error{"prepare input gradients not yet implemented"};
	}

}

template <typename T, typename GRAD_T, uint32_t N_POS_DIMS, uint32_t N_FEATURES_PER_LEVEL, uint32_t N_FEATURES_PER_THREAD>
__global__ void kernel_simplex_backward(
	const uint32_t num_elements,
	const uint32_t num_simplex_features,
	const SimplexOffsetTable offset_table,
	const uint32_t base_resolution,
	const float log2_per_level_scale,
	float max_level,
	const float* __restrict__ max_level_gpu,
	const bool stochastic_interpolation,
	const InterpolationType interpolation_type,
	GRAD_T* __restrict__ simplex_gradient,
	MatrixView<const float> positions_in,
	const T* __restrict__ dL_dy,
	const float f_constant
) {
	const uint32_t i = ((blockIdx.x * blockDim.x + threadIdx.x) * N_FEATURES_PER_THREAD) / N_FEATURES_PER_LEVEL;
	if (i >= num_elements) return;

	const uint32_t level = blockIdx.y ; // <- the level is the same for all threads.
	const uint32_t feature = (blockIdx.x * blockDim.x + threadIdx.x) * N_FEATURES_PER_THREAD - i * N_FEATURES_PER_LEVEL;

	if (max_level_gpu) {
		max_level = (max_level_gpu[i] * num_simplex_features) / N_FEATURES_PER_LEVEL;
	} else {
		max_level = (max_level * num_simplex_features) / N_FEATURES_PER_LEVEL;
	}

	if (level > max_level + 1e-3f) {
		return;
	}

	simplex_gradient += offset_table.data[level] * N_FEATURES_PER_LEVEL;
	const uint32_t hashmap_size = offset_table.data[level + 1] - offset_table.data[level];

	const float scale = simplex_scale(level, log2_per_level_scale, base_resolution);
	const uint32_t resolution = simplex_resolution(scale);

	auto add_simplex_gradient = [&](const uint32_t local_pos[N_POS_DIMS], const vector_t<T, N_FEATURES_PER_THREAD>& grad, const float weight) {
		uint32_t index = simplex_index<N_POS_DIMS>(hashmap_size, resolution, local_pos) * N_FEATURES_PER_LEVEL + feature;
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 600 // atomicAdd(__half2) is only supported with compute capability 60 and above
		if (N_FEATURES_PER_THREAD > 1 && std::is_same<GRAD_T, __half>::value) {
			for (uint32_t f = 0; f < N_FEATURES_PER_THREAD; f += 2) {
				__half2 v = {(__half)((float)grad[f] * weight), (__half)((float)grad[f+1] * weight)};
				atomicAdd((__half2*)&simplex_gradient[index + f], v);
			}
		} else
#endif
		{
			if (std::is_same<GRAD_T, __half>::value) {
				// Should never happen
				// printf("Attempted to use atomicAdd(__half)\n");
			} else {
				for (uint32_t f = 0; f < N_FEATURES_PER_THREAD; ++f) {
					atomicAdd((float*)&simplex_gradient[index + f], (float)grad[f] * weight);
				}
			}
		}
	};

	float pos[N_POS_DIMS];
	uint32_t pos_simplex[N_POS_DIMS];

	float skew = 0;
	TCNN_PRAGMA_UNROLL
	for (uint32_t dim = 0; dim < N_POS_DIMS; ++dim) {
		skew += positions_in(dim, i);
	}
	skew *= f_constant;

	if (interpolation_type == InterpolationType::Nearest || interpolation_type == InterpolationType::Linear) {
		TCNN_PRAGMA_UNROLL
		for (uint32_t dim = 0; dim < N_POS_DIMS; ++dim) {
			pos_fract((positions_in(dim, i) + skew)/std::sqrt(3), &pos[dim], &pos_simplex[dim], scale, identity_fun);
		}
	} else {
		TCNN_PRAGMA_UNROLL
		for (uint32_t dim = 0; dim < N_POS_DIMS; ++dim) {
			pos_fract((positions_in(dim, i) + skew)/std::sqrt(3), &pos[dim], &pos_simplex[dim], scale, smoothstep);
		}
	}

	vector_t<T, N_FEATURES_PER_THREAD> grad;

	TCNN_PRAGMA_UNROLL
	for (uint32_t f = 0; f < N_FEATURES_PER_THREAD; ++f) {
		grad[f] = dL_dy[i + (level * N_FEATURES_PER_LEVEL + feature + f) * num_elements];
	}

	if (interpolation_type == InterpolationType::Nearest) {
		add_simplex_gradient(pos_simplex, grad, 1.0f);
		return;
	}

	if (stochastic_interpolation) {
		float sample = random_val(1337, i + level * num_elements);
		uint32_t pos_simplex_local[N_POS_DIMS];

		TCNN_PRAGMA_UNROLL
		for (uint32_t dim = 0; dim < N_POS_DIMS; ++dim) {
			if (sample >= pos[dim]) {
				pos_simplex_local[dim] = pos_simplex[dim];
			} else {
				pos_simplex_local[dim] = pos_simplex[dim] + 1;
			}
		}

		add_simplex_gradient(pos_simplex_local, grad, 1.0f);
		return;
	}

	// N-linear interpolation
	uint32_t pos_order[N_POS_DIMS];
	simplex_sort<N_POS_DIMS>(pos, pos_order);

	float start_weight = 1;
	uint32_t pos_simplex_local[N_POS_DIMS];

	TCNN_PRAGMA_UNROLL
	for (uint32_t idx = 0; idx < N_POS_DIMS; ++idx) {
		pos_simplex_local[idx] = pos_simplex[idx];
	}

	TCNN_PRAGMA_UNROLL
	for (uint32_t idx = 0; idx < N_POS_DIMS + 1; ++idx) {
		float weight = 0;

		if (idx == N_POS_DIMS) {
			add_simplex_gradient(pos_simplex_local, grad, start_weight);
		} else {
			weight = start_weight - pos[pos_order[idx]];
			add_simplex_gradient(pos_simplex_local, grad, weight);
			pos_simplex_local[pos_order[idx]] += 1; // next simplex vertex

			start_weight = pos[pos_order[idx]];
		}
		
	}

}


// template <typename T>
// __global__ void transpose_encoded_position(
// 	const uint32_t n_elements,
// 	const T* __restrict__ encoded_positions,
// 	PitchedPtr<T> output
// ) {
// 	const uint32_t i = threadIdx.y + blockIdx.x * blockDim.y;
// 	if (i >= n_elements) return;

// 	const uint32_t elem_idx = i;
// 	const uint32_t dim_idx = threadIdx.x;

// 	output(elem_idx)[dim_idx] = encoded_positions[elem_idx + n_elements * dim_idx];
// }

// template <typename T>
// __global__ void transpose_gradients(
// 	const uint32_t n_elements,
// 	T* __restrict__ transposed_dL_dy,
// 	PitchedPtr<const T> dL_dy
// ) {
// 	const uint32_t i = threadIdx.y + blockIdx.x * blockDim.y;
// 	if (i >= n_elements) return;

// 	const uint32_t elem_idx = i;
// 	const uint32_t dim_idx = threadIdx.x;

// 	transposed_dL_dy[elem_idx + n_elements * dim_idx] = dL_dy(elem_idx)[dim_idx];
// }

template <typename T>
class SimplexEncoding : public Encoding<T> {
public:
	virtual uint32_t n_pos_dims() const = 0;
	virtual uint32_t n_features_per_level() const = 0;

	virtual size_t level_n_params(uint32_t level) const = 0;
	virtual size_t level_params_offset(uint32_t level) const = 0;
	
	virtual const SimplexOffsetTable& simplex_offset_table() const = 0;

	float max_level() const {
		return m_max_level;
	}

	void set_max_level(float value) {
		m_max_level = value;
	}

	float* max_level_gpu() const {
		return m_max_level_gpu;
	}

	void set_max_level_gpu(float* value) {
		m_max_level_gpu = value;
	}

	float quantize_threshold() const {
		return m_quantize_threshold;
	}

	void set_quantize_threshold(float value) {
		m_quantize_threshold = value;
	}

protected:
	// Disables lookups of finer levels than this.
	// The default value of 1000 effectively disables the feature
	float m_max_level = 1000.f;

	// If this pointer is non-null, it is expected to point to per-element m_max_level
	float* m_max_level_gpu = nullptr;

	// Features with values less then this threshold are quantized to zero
	float m_quantize_threshold = 0.f;

};

template <typename T, uint32_t N_POS_DIMS=3, uint32_t N_FEATURES_PER_LEVEL=2>
class SimplexEncodingTemplated : public SimplexEncoding<T> {
public:
#if TCNN_MIN_GPU_ARCH >= 62 || TCNN_MIN_GPU_ARCH == 60
	// The GPUs that we tested this on do not have an efficient 1D fp16
	// atomicAdd feature. Thus, we accumulate gradients at fp32 if we're
	// forced to use 1D atomicAdds. As soon as 2D or higher is possible,
	// we can make use the efficient atomicAdd(half2) function.
	using grad_t = std::conditional_t<N_FEATURES_PER_LEVEL == 1, float, T>;
#else
	// atomicAdd(__half2) is only supported with compute capability 60 and above.
	// Since atomicAdd(__half) is relatively slow / doesn't exist for low compute
	// capabilities, accumulate in fp32 instead.
	using grad_t = float;
#endif

	SimplexEncodingTemplated(
		uint32_t n_features,
		uint32_t log2_hashmap_size,
		uint32_t base_resolution,
		float per_level_scale,
		bool stochastic_interpolation,
		InterpolationType interpolation_type
	) :
		m_n_features{n_features},
		m_log2_hashmap_size{log2_hashmap_size},
		m_base_resolution{get_adjusted_base_resolution(N_POS_DIMS, base_resolution)},
		m_per_level_scale{per_level_scale},
		m_stochastic_interpolation{stochastic_interpolation},
		m_interpolation_type{interpolation_type},
		m_f_constant{(std::sqrt(N_POS_DIMS+1.0f)-1.0f) / N_POS_DIMS} // calculate F
	{
		m_n_levels = div_round_up(m_n_features, N_FEATURES_PER_LEVEL);
		uint32_t offset = 0;

		if (m_n_levels > MAX_N_LEVELS) {
			throw std::runtime_error{fmt::format("SimplexEncoding: m_n_levels={} must be at most MAX_N_LEVELS={}", m_n_levels, MAX_N_LEVELS)};
		}
		
		for (uint32_t i = 0; i < m_n_levels; ++i) {
			// Compute number of dense params required for the given level
			const uint32_t resolution = simplex_resolution(simplex_scale(i, std::log2(per_level_scale), m_base_resolution));

			uint32_t max_params = std::numeric_limits<uint32_t>::max()/2;
			uint32_t params_in_level = std::pow((float)resolution, N_POS_DIMS) > (float)max_params ? max_params : powi(resolution, N_POS_DIMS);

			// Make sure memory accesses will be aligned
			params_in_level = next_multiple(params_in_level, 8u);

			// If hash table needs fewer params than dense, then use fewer and rely on the hash.
			params_in_level = std::min(params_in_level, (1u << log2_hashmap_size));

			m_offset_table.data[i] = offset;
			offset += params_in_level;

#ifdef TCNN_VERBOSE_MEMORY_ALLOCS
			std::cout << "SimplexEncoding at level " << i << ": resolution=" << resolution << " params_in_level=" << params_in_level << std::endl;
#endif
		}

		m_offset_table.data[m_n_levels] = offset;
		m_offset_table.size = m_n_levels+1;

		m_n_params = m_offset_table.data[m_n_levels] * N_FEATURES_PER_LEVEL;

		m_n_output_dims = m_n_features;

		if (n_features % N_FEATURES_PER_LEVEL != 0) {
			throw std::runtime_error{fmt::format("SimplexEncoding: n_features={} must be a multiple of N_FEATURES_PER_LEVEL={}", n_features, N_FEATURES_PER_LEVEL)};
		}

	}

	std::unique_ptr<Context> forward_impl(
		cudaStream_t stream,
		const GPUMatrixDynamic<float>& input,
		GPUMatrixDynamic<T>* output = nullptr,
		bool use_inference_params = false,
		bool prepare_input_gradients = false
	) override {
		
		auto forward = std::make_unique<ForwardContext>();

		const uint32_t num_elements = input.n();
		if ((!output && !prepare_input_gradients) || padded_output_width() == 0 || num_elements == 0) {
			return forward;
		}

		SyncedMultiStream synced_streams{stream, m_n_to_pad > 0 ? 2u : 1u};

		// Take care of padding on the auxiliary stream
		if (output && m_n_to_pad > 0) {
			if (output->layout() == AoS) {
				parallel_for_gpu_aos(synced_streams.get(1), num_elements, m_n_to_pad, [n_output_dims=m_n_output_dims, out=output->pitched_ptr()] __device__ (size_t elem, size_t dim) {
					out(elem)[n_output_dims + dim] = 0;
				});
			} else {
				parallel_for_gpu(synced_streams.get(1), num_elements * m_n_to_pad, [out=output->data() + num_elements * m_n_output_dims] __device__ (size_t i) {
					out[i] = 0;
				});
			}
		}

		// Idea: each block only takes care of _one_ hash level (but may iterate over multiple input elements).
		// This way, only one level of the hashmap needs to fit into caches at a time (and it reused for consecutive
		// elements) until it is time to process the next level.
		static constexpr uint32_t N_THREADS_HASHSIMPLEX = 512;
		const dim3 blocks_hashsimplex = { div_round_up(num_elements, N_THREADS_HASHSIMPLEX), m_n_levels, 1 };

		T* encoded_positions_soa = output ? output->data() : nullptr;
		GPUMemoryArena::Allocation workspace;
		if (output && output->layout() == AoS) {
			workspace = allocate_workspace(stream, num_elements * m_n_features * sizeof(T));
			encoded_positions_soa = (T*)workspace.data();
		}

		if (prepare_input_gradients) {
			throw std::runtime_error{"prepare input gradients not yet implemented in Simplex Encoding"};
			forward->dy_dx = GPUMatrix<float, RM>{N_POS_DIMS * m_n_features, input.n(), stream};
		}

		kernel_simplex<T, N_POS_DIMS, N_FEATURES_PER_LEVEL><<<blocks_hashsimplex, N_THREADS_HASHSIMPLEX, 0, synced_streams.get(0)>>>(
			num_elements,
			m_n_features,
			m_offset_table,
			m_base_resolution,
			std::log2(m_per_level_scale),
			this->m_quantize_threshold,
			this->m_max_level,
			this->m_max_level_gpu,
			m_interpolation_type,
			use_inference_params ? this->inference_params() : this->params(),
			forward->positions.data() ? forward->positions.view() : input.view(),
			encoded_positions_soa,
			forward->dy_dx.data(),
			this->m_f_constant
		);

		if (output && output->layout() == AoS) {
			// Transpose result (was stored row major due to coalescing)
			const dim3 threads_transpose = { m_n_levels * N_FEATURES_PER_LEVEL, 8, 1 };
			const uint32_t blocks_transpose = div_round_up(num_elements, threads_transpose.y);
			transpose_encoded_position<T><<<blocks_transpose, threads_transpose, 0, synced_streams.get(0)>>>(
				num_elements,
				encoded_positions_soa,
				output->pitched_ptr()
			);
		}

		return forward;

	}

	void backward_impl(
		cudaStream_t stream,
		const Context& ctx,
		const GPUMatrixDynamic<float>& input,
		const GPUMatrixDynamic<T>& output,
		const GPUMatrixDynamic<T>& dL_doutput,
		GPUMatrixDynamic<float>* dL_dinput = nullptr,
		bool use_inference_params = false,
		EGradientMode param_gradients_mode = EGradientMode::Overwrite
	) override {
		const uint32_t num_elements = input.n();
		if ((!dL_dinput && param_gradients_mode == EGradientMode::Ignore) || padded_output_width() == 0 || num_elements == 0) {
			return;
		}

		const auto& forward = dynamic_cast<const ForwardContext&>(ctx);

		const T* dL_dy_rm = dL_doutput.data();

		GPUMemoryArena::Allocation workspace;
		if (dL_doutput.layout() == CM) {
			workspace = allocate_workspace(stream, num_elements * m_n_features * sizeof(T));

			// Transpose dL_dy. Use the buffer previously occupied by the encoded positions
			const dim3 threads_transpose = { m_n_levels * N_FEATURES_PER_LEVEL, 8, 1 };
			const uint32_t blocks_transpose = div_round_up(num_elements, threads_transpose.y);
			transpose_gradients<T><<<blocks_transpose, threads_transpose, 0, stream>>>(
				num_elements,
				(T*)workspace.data(),
				dL_doutput.pitched_ptr()
			);

			dL_dy_rm = (const T*)workspace.data();
		}

		if (param_gradients_mode != EGradientMode::Ignore) {
			// We accumulate gradients with grad_t precision, which, for performance reasons, is not always T.
			// If not, accumulate in a temporary buffer and cast later.
			grad_t* simplex_gradient;
			GPUMemoryArena::Allocation simplex_gradient_tmp;

			if (!std::is_same<grad_t, T>::value) {
				simplex_gradient_tmp = allocate_workspace(stream, m_n_params * sizeof(grad_t));
				simplex_gradient = (grad_t*)simplex_gradient_tmp.data();
			} else {
				simplex_gradient = (grad_t*)this->gradients();
			}

			if (param_gradients_mode == EGradientMode::Overwrite) {
				CUDA_CHECK_THROW(cudaMemsetAsync(simplex_gradient, 0, n_params() * sizeof(grad_t), stream));
			}

			static constexpr uint32_t N_THREADS_HASHSIMPLEX = 256;
			static constexpr uint32_t N_FEATURES_PER_THREAD = std::min(2u, N_FEATURES_PER_LEVEL);

			const dim3 blocks_hashsimplex = { div_round_up(num_elements * N_FEATURES_PER_LEVEL / N_FEATURES_PER_THREAD, N_THREADS_HASHSIMPLEX), m_n_levels, 1 };

			kernel_simplex_backward<T, grad_t, N_POS_DIMS, N_FEATURES_PER_LEVEL, N_FEATURES_PER_THREAD><<<blocks_hashsimplex, N_THREADS_HASHSIMPLEX, 0, stream>>>(
				num_elements,
				m_n_features,
				m_offset_table,
				m_base_resolution,
				std::log2(m_per_level_scale),
				this->m_max_level,
				this->m_max_level_gpu,
				m_stochastic_interpolation,
				m_interpolation_type,
				simplex_gradient,
				forward.positions.data() ? forward.positions.view() : input.view(), // positions SoA
				dL_dy_rm, // gradients SoA
				this->m_f_constant
			);

			if (!std::is_same<grad_t, T>::value) {
				parallel_for_gpu(stream, n_params(), [grad=this->gradients(), grad_tmp=simplex_gradient] __device__ (size_t i) {
					grad[i] = (T)grad_tmp[i];
				});
			}
		}

		if (!dL_dinput) {
			return;
		}

		throw std::runtime_error("kernel_simplex_backward_input not yet implemented");

		// linear_kernel(kernel_simplex_backward_input<T, N_POS_DIMS>, 0, stream,
		// 	num_elements,
		// 	m_n_features,
		// 	dL_dy_rm,
		// 	forward.dy_dx.data(),
		// 	dL_dinput->view()
		// );

	}

	uint32_t input_width() const override {
		return N_POS_DIMS;
	}

	uint32_t padded_output_width() const override {
		return m_n_output_dims + m_n_to_pad;
	}

	uint32_t output_width() const override {
		return padded_output_width();
	}

	uint32_t required_input_alignment() const override {
		return 1;
	}

	void set_padded_output_width(uint32_t padded_output_width) override {
		CHECK_THROW(padded_output_width >= m_n_output_dims);
		m_n_to_pad = padded_output_width - m_n_output_dims;
	}

	uint32_t required_output_alignment() const override {
		return N_FEATURES_PER_LEVEL;
	}

	MatrixLayout preferred_output_layout() const override {
		return SoA;
	}

	void set_params_impl(T* params, T* inference_params, T* gradients) override { }

	void initialize_params(pcg32& rnd, float* params_full_precision, float scale = 1) override {
		// Initialize the hashsimplex from the GPU, because the number of parameters can be quite large.
		generate_random_uniform<float>(rnd, n_params(), params_full_precision, -1e-4f * scale, 1e-4f * scale);
	}

	size_t n_params() const override {
		return m_n_params;
	}

	size_t level_n_params(uint32_t level) const override {
		return level_params_offset(level + 1) - level_params_offset(level);
	}

	size_t level_params_offset(uint32_t level) const override {
		if (level >= m_offset_table.size) {
			throw std::runtime_error{"Out of bounds params offset request."};
		}

		return m_offset_table.data[level];
	}

	const SimplexOffsetTable& simplex_offset_table() const override {
		return m_offset_table;
	}

	std::vector<std::pair<uint32_t, uint32_t>> layer_sizes() const override {
		// Even though we have parameters, they can't really be considered a "layer".
		// So we return an empty array here.
		return {};
	}

	uint32_t n_pos_dims() const override {
		return N_POS_DIMS;
	}

	uint32_t n_features_per_level() const override {
		return N_FEATURES_PER_LEVEL;
	}

	json hyperparams() const override {
		json result = {
			{"otype", "Simplex"},
			{"n_levels", m_n_levels},
			{"n_features_per_level", N_FEATURES_PER_LEVEL},
			{"base_resolution", get_unadjusted_base_resolution(n_pos_dims(), m_base_resolution)},
			{"per_level_scale", m_per_level_scale},
			{"log2_hashmap_size", m_log2_hashmap_size},
			{"interpolation", to_string(m_interpolation_type)},
		};

		return result;
	}

private:
	struct ForwardContext : public Context {
		GPUMatrix<float, RM> positions;
		GPUMatrix<float, RM> dy_dx;
	};

	uint32_t m_n_features;
	uint32_t m_n_levels;
	uint32_t m_n_params;
	SimplexOffsetTable m_offset_table;
	uint32_t m_log2_hashmap_size;
	uint32_t m_base_resolution; // uint32_t to float

	uint32_t m_n_dims_to_pass_through;

	// derived sizes
	uint32_t m_n_output_dims;
	uint32_t m_n_to_pad = 0;

	float m_per_level_scale;

	bool m_stochastic_interpolation;
	InterpolationType m_interpolation_type;

	const float m_f_constant;

};

template <typename T, uint32_t N_FEATURES_PER_LEVEL>
SimplexEncoding<T>* create_simplex_encoding_templated_1(uint32_t n_dims_to_encode, const json& encoding) {
	const uint32_t log2_hashmap_size = encoding.value("log2_hashmap_size", 19u);
	const std::string encoding_type = encoding.value("otype", "Simplex");

	uint32_t n_features;
	if (encoding.contains("n_features") || encoding.contains("n_simplex_features")) {
		n_features = encoding.contains("n_features") ? encoding["n_features"] : encoding["n_simplex_features"];
		if (encoding.contains("n_levels")) {
			throw std::runtime_error{"SimplexEncoding: may not specify n_features and n_levels simultaneously (one determines the other)"};
		}
	} else {
		n_features = N_FEATURES_PER_LEVEL * encoding.value("n_levels", 16u);
	}

#define TCNN_SIMPLEX_PARAMS \
	n_features, \
	log2_hashmap_size, \
	encoding.value("base_resolution", 16u), \
	encoding.value("per_level_scale", 2.0f), \
	encoding.value("stochastic_interpolation", false), \
	string_to_interpolation_type(encoding.value("interpolation", "Linear")), \

	// If higher-dimensional hash encodings are desired, corresponding switch cases can be added
	switch (n_dims_to_encode) {
		// case 1: return new SimplexEncodingTemplated<T, 1, N_FEATURES_PER_LEVEL>{ TCNN_SIMPLEX_PARAMS };
		case 2: return new SimplexEncodingTemplated<T, 2, N_FEATURES_PER_LEVEL>{ TCNN_SIMPLEX_PARAMS };
		case 3: return new SimplexEncodingTemplated<T, 3, N_FEATURES_PER_LEVEL>{ TCNN_SIMPLEX_PARAMS };
		case 4: return new SimplexEncodingTemplated<T, 4, N_FEATURES_PER_LEVEL>{ TCNN_SIMPLEX_PARAMS };
		// case 5: return new SimplexEncodingTemplated<T, 5, N_FEATURES_PER_LEVEL>{ TCNN_SIMPLEX_PARAMS };
		// case 6: return new SimplexEncodingTemplated<T, 6, N_FEATURES_PER_LEVEL>{ TCNN_SIMPLEX_PARAMS };
		// case 7: return new SimplexEncodingTemplated<T, 7, N_FEATURES_PER_LEVEL>{ TCNN_SIMPLEX_PARAMS };
		default: throw std::runtime_error{"SimplexEncoding: number of input dims must be 2 or 3."};
	}
#undef TCNN_SIMPLEX_PARAMS
}

template <typename T>
SimplexEncoding<T>* create_simplex_encoding(uint32_t n_dims_to_encode, const json& encoding) {
	const uint32_t n_features_per_level = encoding.value("n_features_per_level", 2u);
	switch (n_features_per_level) {
		case 1: return create_simplex_encoding_templated_1<T, 1>(n_dims_to_encode, encoding);
		case 2: return create_simplex_encoding_templated_1<T, 2>(n_dims_to_encode, encoding);
		case 4: return create_simplex_encoding_templated_1<T, 4>(n_dims_to_encode, encoding);
		case 8: return create_simplex_encoding_templated_1<T, 8>(n_dims_to_encode, encoding);
		default: throw std::runtime_error{"SimplexEncoding: n_features_per_level must be 1, 2, 4, or 8."};
	}
}

TCNN_NAMESPACE_END